...
# --- MOCK HOME ASSISTANT ---
# Mocka HA-moduler INNAN vi importerar komponenten för att slippa installera 'homeassistant'
mock_hass = MagicMock()
...
from custom_components.battery_optimizer_light.coordinator import BatteryOptimizerLightCoordinator
from custom_components.battery_optimizer_light import PeakGuard
...
@pytest.mark.asyncio
async def test_coordinator_handles_unavailable_soc(mock_hass):
    """Krav: Om SoC är otillgänglig ska vi INTE anropa API:et (för att undvika skräpdata)."""
    coordinator = BatteryOptimizerLightCoordinator(mock_hass, MOCK_CONFIG)
...
@pytest.mark.asyncio
async def test_peak_guard_triggers_discharge(mock_hass):
    ...
    guard = PeakGuard(mock_hass, MOCK_CONFIG, coordinator)
    ...
    def get_state_side_effect(entity_id):
        if entity_id == "sensor.optimizer_light_peak_limit": return limit_state
        if entity_id == "sensor.husets_netto_last_virtuell": return load_state
        if entity_id == "sensor.soc": return soc_state
        return None
...
@pytest.mark.asyncio
async def test_peak_guard_respects_safe_limit(mock_hass):
    ...
    limit_state = MagicMock(); limit_state.state = "5.0"
    # Last: 3 kW (Väl under safe limit)
    load_state = MagicMock(); load_state.state = "3000"
    soc_state = MagicMock(); soc_state.state = "50"
